...
def generar_muestra(size):
    ...
    muestra = np.random.randint(0, 1501, sizes[size])  #ordenamos para poder dividir los rangos y que sean excluyentes mas adelante
    muestra.sort()
...
def tabla(muestra, rangos):
    ...
    for i, rango_particiones in enumerate(rangos):
        ...
        print(f"{i:<2} || {rango_particiones[0]:<3} - {rango_particiones[1]:<6} || {frecuencia_absoluta:<6} || {frecuencia_relativa:.2f}")
